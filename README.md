...
| `%APPDATA%\Zed\settings.json`, `keymap.json` | 예 | 아니요 | 아니요 |
| `~/.config/zed/settings.json`, `keymap.json` | 아니요 | 예 | 예 |
...
Windows에서는 `%APPDATA%\Zed`, macOS와 Linux에서는 `~/.config/zed`에
배치한다. 실제 설정 본문은 `home/.chezmoitemplates/zed-*.json.tmpl`에서
한 번만 관리하며 플랫폼별 대상 템플릿은 이 공통 본문을 렌더링한다.
...
```sh
cd "$HOME/.dotfiles"
${EDITOR:-vi} home/.chezmoitemplates/zed-settings.json.tmpl
chezmoi diff
chezmoi apply --verbose
...
공통 Zed 템플릿은 플랫폼별 1줄 대상 템플릿이 아니라 위 경로를 직접
편집한다. 그 밖에 `chezmoi edit`는 대상 경로에 대응하는 소스를 편집한다. 루트
...
`chezmoi re-add`는 템플릿을 덮어쓰지 않는다. Ghostty, Zsh처럼
...
Zed는 플랫폼별 대상 템플릿이 공통 템플릿을 호출하므로 `chezmoi merge` 대신
`home/.chezmoitemplates/zed-settings.json.tmpl` 또는
`home/.chezmoitemplates/zed-keymap.json.tmpl`에 앱의 변경을 수동으로 반영한다.

병합 대신 `~/.dotfiles/home/`의 대응 템플릿에 수동으로 반영해도
된다. Zed는 `home/.chezmoitemplates/zed-settings.json.tmpl`, Ghostty는
