...
assert_readme_contains 'chezmoi edit "$HOME/.config/zed/settings.json"' \
...
assert_readme_contains 'chezmoi merge "$HOME/.config/zed/settings.json"' \
  "템플릿 대상 merge 명령이 README에 없습니다."
...
assert_readme_contains 'git diff -- home/dot_config/zed/settings.json.tmpl' \
  "merge 후 소스 템플릿 확인 명령이 README에 없습니다."
...
merge_line=$(readme_line_after 0 'chezmoi merge "$HOME/.config/zed/settings.json"')
source_diff_line=$(readme_line_after "$merge_line" 'git diff -- home/dot_config/zed/settings.json.tmpl')
[ -n "$source_diff_line" ] || fail "merge 직후 소스 템플릿 확인 순서가 README에 없습니다."
target_diff_line=$(readme_line_after "$source_diff_line" 'chezmoi diff "$HOME/.config/zed/settings.json"')
[ -n "$target_diff_line" ] || fail "소스 확인 후 대상 diff 순서가 README에 없습니다."
apply_line=$(readme_line_after "$target_diff_line" 'chezmoi apply --verbose')
[ -n "$apply_line" ] || fail "대상 diff 후 apply 순서가 README에 없습니다."
